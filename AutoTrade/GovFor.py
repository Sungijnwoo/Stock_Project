...
class Cp7210:
    ...
    def request(self, investFlag, caller):
        ...
        while True:
            self.objRq.SetInputValue(0, 'A011040')   #종목 코드 - 삼성전자
            self.objRq.SetInputValue(1, ord('0')) # 개수로 조회
            self.objRq.SetInputValue(2, investFlag) # 0 종목 1 외국인 2 기관계 3 보험기타 4 투신..
            self.objRq.SetInputValue(3, ord('0'))
            self.objRq.SetInputValue(4, 10) # 최근 100일 치
            self.objRq.SetInputValue(5, [0,2,3,4,5, 8]) #날짜,시가,고가,저가,종가,거래량
            self.objRq.SetInputValue(6, ord('D')) # '차트 주가 - 일간 차트 요청
            self.objRq.SetInputValue(9, ord('1')) # 수정주가 사용
